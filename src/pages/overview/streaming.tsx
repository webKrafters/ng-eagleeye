...
const StreamingPage : React.FC<{className? : string}> = ({ className }) => (
    <article className={ `streaming-page ${ className }` }>
		...
		<h3>Streaming</h3>
		<ListItem><div>accepts an optional <Anchor to="/concepts/selector-map">selector map</Anchor>; and derives a change stream context <Anchor to="/concepts/store">store.data</Anchor> of derived signal fields.</div></ListItem>
		...
		<Paragraph>A <code>StreamService</code> instance streams to a <Anchor to="/concepts/client">client</Anchor> all changes occuring within an <Name /> context store.</Paragraph>
		<Paragraph>A <code>StreamService</code> instance holds <b>3 <Anchor to="/concepts/store">store</Anchor></b> properties.</Paragraph>
        ...
        <Paragraph><CodeBlock>{ stream_sample }</CodeBlock></Paragraph>
		<LiveCodeToggle />
    </article>
);
